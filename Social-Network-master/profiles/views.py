...
def search_profile(request):
    ...
    if len(query) > 78:
        allProfiles = Profile.objects.none()
    else:
        allname = Profile.objects.filter(first_name__icontains=query)
        alllast_name = Profile.objects.filter(last_name__icontains=query)
        allbio = Profile.objects.filter(bio__icontains=query)
        allProfiles = allname.union(alllast_name, allbio)
    context = {
        'query': query,
